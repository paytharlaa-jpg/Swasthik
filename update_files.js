...
const layoutSearch = export const metadata: Metadata = {
...
const layoutReplace = export const metadata: Metadata = {
...
replaceInFile(layoutPath, layoutSearch, layoutReplace);

const layoutSearch2 = <LenisProvider>
          <Header />
          {children}
        </LenisProvider>;
const layoutReplace2 = <LenisProvider>
          <Header />
          {children}
...
replaceInFile(layoutPath, layoutSearch2, layoutReplace2);


// 2. Update Header.tsx
const headerPath = './components/layout/Header.tsx';
const headerSearch = <button className="hidden md:block border border-gold text-gold px-6 py-2 rounded-full uppercase tracking-widest text-xs font-semibold hover:bg-gold hover:text-midnight transition-colors">
          Book Consultation
        </button>;
const headerReplace = <a 
          href="https://api.whatsapp.com/send/?phone=918367495666&text=Hello%2521%2520I%2520want%2520to%2520book%2520Free%2520Consultation%2520and%2520want%2520to%2520know%2520more%2520about%2520Offer&type=phone_number&app_absent=0&utm_source=ig"
          target="_blank"
          rel="noopener noreferrer"
...
replaceInFile(headerPath, headerSearch, headerReplace);
